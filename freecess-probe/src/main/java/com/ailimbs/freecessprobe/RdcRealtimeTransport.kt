 ...
 */
internal class RdcRealtimeTransport(
    ...
    private val listener = object : WebSocketListener() {
        ...
        override fun onMessage(webSocket: WebSocket, text: String) {
            val message = runCatching { JSONObject(text) }.getOrElse { error ->
                ProbeLog.w(TAG, "Ignoring invalid RDC Realtime frame", error)
                return
            }
            ...
            when (event) {
                ...
                "phx_error" -> {
                    if (topicName == legacyTopic) {
                        ...
                        ProbeLog.w(TAG, "RDC legacy Postgres Changes channel reported phx_error; primary doorbell remains active")
                    } else {
                ...
                "phx_close" -> if (!closing) {
                    if (topicName == legacyTopic) {
                        legacyReadyState = false
                        ProbeLog.w(TAG, "RDC legacy Postgres Changes channel closed; primary doorbell remains active")
                    } else {
    ...
    private fun handleReply(ref: String, payload: JSONObject) {
        ...
        when (ref) {
            ...
            presenceRef -> {
                ...
                readyState = true
                ready.complete(Unit)
                ProbeLog.i(TAG, "RDC Realtime private channel and Presence are ready")
            }
            legacyJoinRef -> {
                legacyReadyState = ok
                if (ok) {
                    ...
                    ProbeLog.w(TAG, "RDC legacy Postgres Changes join was rejected; primary doorbell remains active: $payload")
                }
            }
    ...
    private fun sendLegacyJoin() {
        ...
        if (!sendFrame(legacyTopic, "phx_join", payload, newLegacyJoinRef, newLegacyJoinRef)) {
            legacyJoinRef = null
            legacyReadyState = false
            ProbeLog.w(TAG, "Unable to send RDC legacy Postgres Changes phx_join; primary doorbell remains active")
        }
    }
    ...
    private fun handlePostgresChange(topicName: String, payload: JSONObject) {
        ...
        if (data == null) {
            ProbeLog.w(TAG, "RDC postgres_changes frame missing data object")
            return
        }
        ...
        if (record == null) {
            ProbeLog.w(TAG, "RDC postgres_changes INSERT missing record object")
            return
        }
        val callId = record.optString("id")
        val targetDeviceId = record.optString("device_id")
        if (callId.isBlank()) {
            ProbeLog.w(TAG, "RDC postgres_changes INSERT missing call id")
            return
        }
    ...
    private fun fail(error: Throwable) {
        ...
        pendingAcks.values.forEach { it.complete(false) }
        pendingAcks.clear()
        ProbeLog.e(TAG, "RDC Realtime transport failed", error)
    }
