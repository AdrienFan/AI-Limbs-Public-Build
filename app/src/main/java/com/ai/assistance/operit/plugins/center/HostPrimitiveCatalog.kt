...
object AiLimbsHostPrimitiveCatalog {
    val all: List<HostPrimitiveDefinition> = listOf(
        ...
        HostPrimitiveDefinition(5, "host.screen.capture@1", "Screen Capture", "获取设备屏幕或显示目标的单帧截图/原始画面帧。", "Host 负责单帧屏幕捕获及必要系统授权；持续屏幕共享/MediaProjection 会话属于 host.screen.session@1，压缩、OCR、视觉推理、缓存和归档由插件完成。", HostPrimitiveMaturity.CONFIRMED, HostPrimitiveExposure.BOUND, true),
        HostPrimitiveDefinition(6, "host.network@1", "Network I/O", "提供受控网络连接、流式收发与只读监听端口快照，并实施端点、代理、TLS 和监听端口策略。", "当前业务 runtime owner 管网络权限和连接资源；Resident BUSINESS 下由 Core 执行，listeners 只暴露当前监听 TCP 端口，不暴露任意 Shell；HTTP、WebSocket、Webhook 等具体协议和业务语义由插件实现。", HostPrimitiveMaturity.CONFIRMED, HostPrimitiveExposure.BOUND, true),
        ...
        HostPrimitiveDefinition(17, "host.android.component@1", "Android Component Invocation", "受控启动 Activity、发送 Broadcast、调用 Service，并处理 Intent/URI/extras。", "Host 校验目标、参数和风险；Tasker、拍照、打开设置、分享等业务语义由插件组合。", HostPrimitiveMaturity.CONFIRMED, HostPrimitiveExposure.BOUND, true),
        ...
        HostPrimitiveDefinition(23, "host.logging@1", "Structured Logging", "提供动态日志来源枚举、按来源读取/导出、全局清理以及身份绑定日志写入。", "Host 绑定真实插件/子插件身份并管理统一日志 sink；日志中心只消费 sources/read/export/clear，不维护组件名单或宿主日志物理路径。", HostPrimitiveMaturity.CONFIRMED, HostPrimitiveExposure.BOUND, true),
        ...
        HostPrimitiveDefinition(39, "host.camera.capture@1", "Camera Capture / Visual Sensor", "在用户授权下从指定摄像头获取单帧照片/原始画面。", "Host 管 CAMERA 权限、单帧 capture 生命周期和受控结果 handle；持续摄像头占用与视觉会话属于 host.camera.session@1，插件不直接取得 Activity/Context/CameraManager。", HostPrimitiveMaturity.CONFIRMED, HostPrimitiveExposure.BOUND, true),
        ...
        HostPrimitiveDefinition(47, "host.interaction.cycle@1", "Interaction Cycle", "读取、设置、手动刷新、释放本轮门禁或显式结束 AI Limbs 外部交互周期。", "release_gate 仅撤销当前 generation 的 Bootstrap/receipt/WORK 门禁，不结束 Interaction Cycle；close 才结束当前周期。周期边界仍由 AI Limbs 基座执行。", HostPrimitiveMaturity.CONFIRMED, HostPrimitiveExposure.BOUND, false),
