...
object AiLimbsHostPrimitiveCatalog {
    val all: List<HostPrimitiveDefinition> = listOf(
        ...
        HostPrimitiveDefinition(6, "host.network@1", "Network I/O", "提供受控网络连接、流式收发与只读监听端口快照，并实施端点、代理、TLS 和监听端口策略。", "Host 管网络权限和连接资源；listeners 只暴露当前监听 TCP 端口，不暴露任意 Shell；HTTP、WebSocket、Webhook 等具体协议和业务语义由插件实现。", HostPrimitiveMaturity.CONFIRMED, HostPrimitiveExposure.BOUND, true),
        ...
        HostPrimitiveDefinition(50, "host.resident.runtime@1", "AI Limbs Resident Runtime", "管理 AI Limbs Resident 开关、Guardian 与独立核心验证入口；持续工作效果须经实机验证。", "开启请求 Resident 常驻与宿主 WakeLock，不代表系统已豁免冻结。core_probe/core_status/core_stop 仅供 Plugin Center 验证独立 Context、主 Looper、Core 运行时骨架和 IPC；尚未迁移插件或业务所有权，不向插件暴露 Shell 或 Binder。", HostPrimitiveMaturity.CONFIRMED, HostPrimitiveExposure.BOUND, false),
