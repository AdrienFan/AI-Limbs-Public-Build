...
internal interface PluginCapabilityInvokerFactory {
    fun create(ownerPluginId: String, grantedScopes: Set<String>): PluginCapabilityInvoker
}
...
class PluginPermissionGate internal constructor(scopes: Set<String>) {
    val grantedScopes: Set<String> = scopes.toSortedSet()

    fun isGranted(scope: String): Boolean = scope.trim().lowercase() in grantedScopes

    fun require(scope: String) {
        val normalized = scope.trim().lowercase()
        if (normalized !in grantedScopes) {
            throw PluginInstallException("PLUGIN_SCOPE_DENIED", "Plugin scope was not granted: $normalized")
        }
    }
}
...
class PluginContext internal constructor(
    ...
    val permissions: PluginPermissionGate
)
...
internal class PluginContextFactory(
    ...
    fun create(
        ...
        PluginContext(
            ...
            permissions = PluginPermissionGate(grantedScopes)
        )
}
