...
class ExternalChatHttpServer(
    ...
    override fun serve(session: IHTTPSession): Response {
        return when {
            ...
            (session.uri == AI_LIMBS_HEALTH_PATH || session.uri == LEGACY_LANER_HEALTH_PATH) && session.method == Method.GET -> handleAiLimbsHealth(session)
            (session.uri == AI_LIMBS_TOOL_PATH || session.uri == LEGACY_LANER_TOOL_PATH) && session.method == Method.POST -> handleAiLimbsTool(session)
    ...
    private fun handleAiLimbsHealth(session: IHTTPSession): Response {
    ...
    private fun handleAiLimbsTool(session: IHTTPSession): Response {
        requireLoopback(session)?.let { return it }
        requireBearerToken(session)?.let { return it }

        val requestBodyResult = readRequestBody(session)
        if (requestBodyResult.error != null) {
            return lanerError(Response.Status.BAD_REQUEST, requestBodyResult.error)
        }

        val request = try {
            JSONObject(requestBodyResult.body.orEmpty())
        } catch (e: Exception) {
            return lanerError(Response.Status.BAD_REQUEST, "Invalid JSON: ${e.message}")
        }

        val tool = request.optString("tool").trim()
        if (tool.isBlank()) {
    ...
    private fun requireLoopback(session: IHTTPSession): Response? {
        val address = session.remoteIpAddress.orEmpty()
        val isLoopback = address == "127.0.0.1" || address == "::1" || address == "0:0:0:0:0:0:0:1"
        return if (isLoopback) null else lanerError(Response.Status.FORBIDDEN, "AI Limbs diagnostic HTTP endpoints are loopback-only")
    }

    private fun lanerError(status: Response.Status, message: String): Response {
        return rawJsonResponse(status, JSONObject().put("success", false).put("error", message)).withCors()
    }
    ...
    private fun rawJsonResponse(status: Response.Status, body: JSONObject): Response {
        return newFixedLengthResponse(status, JSON_MIME_TYPE, body.toString())
    }
    ...
    companion object {
        ...
        private const val AI_LIMBS_HEALTH_PATH = "/api/ai-limbs/health"
        private const val AI_LIMBS_TOOL_PATH = "/api/ai-limbs/tool"
        private const val LEGACY_LANER_HEALTH_PATH = "/api/laner/health"
        private const val LEGACY_LANER_TOOL_PATH = "/api/laner/tool"
