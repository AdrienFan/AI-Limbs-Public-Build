#!/usr/bin/env python3
"""Build the signed Visual Manager .ailp from the committed manifest and payload APK."""
...
def main() -> None:
    apks = list(
        (ROOT / "plugin-lab/plugins/visual-manager/build/outputs/apk/debug").glob("*.apk")
    )
    if len(apks) != 1:
        raise SystemExit("Expected exactly one Visual Manager APK")
    apk = apks[0]

    manifest_path = ROOT / "plugin-lab/packages/visual-manager/plugin.json"
    ...
    with tempfile.TemporaryDirectory(prefix="ail-visual-manager-sign-") as temporary:
        work = Path(temporary)
        ...
        output = dist / f"AI-Limbs-Visual-Manager-v{manifest['version']}.ailp"
