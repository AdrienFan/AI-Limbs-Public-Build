package com.ai.limbs.plugins.systemenvironment.subsystems.ubuntu.runtime.terminal.view.canvas
...
import com.ai.limbs.plugins.systemenvironment.subsystems.ubuntu.runtime.terminal.R
import com.ai.limbs.plugins.systemenvironment.subsystems.ubuntu.runtime.terminal.SharedHiddenTerminalState
 import com.ai.limbs.plugins.systemenvironment.subsystems.ubuntu.runtime.terminal.view.domain.ansi.AnsiTerminalEmulator
 import com.ai.limbs.plugins.systemenvironment.subsystems.ubuntu.runtime.terminal.view.domain.ansi.TerminalChar
 import java.io.File
...
class CanvasTerminalView @JvmOverloads constructor(
    ...
    private var onSharedView: (() -> Unit)? = null
    @Volatile
    private var sharedViewActive: Boolean = false
    @Volatile
    private var sharedViewSelected: Boolean = false
    @Volatile
    private var sharedTerminalState = SharedHiddenTerminalState()
    @Volatile
    ...
    // PTY 引用（用于窗口大小同步）
    private var pty: com.ai.limbs.plugins.systemenvironment.subsystems.ubuntu.runtime.terminal.Pty? = null
    ...
    private data class TabHitSnapshot(
        ...
        val sharedButtonRect: RectF = RectF()
    )
    ...
    private sealed interface TabTouchTarget {
        ...
        data object SharedView : TabTouchTarget
     ...
     * 设置 PTY（用于窗口大小同步）
     */
    fun setPty(pty: com.ai.limbs.plugins.systemenvironment.subsystems.ubuntu.runtime.terminal.Pty?) {
        this.pty = pty
    ...
    fun setTabBarState(
        ...
        this.onSharedView = onSharedView
        this.sharedViewActive = sharedViewActive
        if (!hasTabBar()) {
    ...
    fun setSharedTerminalViewState(
        selected: Boolean,
        state: SharedHiddenTerminalState
    ) {
        if (sharedViewSelected == selected && sharedTerminalState == state) return
        sharedViewSelected = selected
    ...
    private fun hasTabBar(): Boolean = tabs.isNotEmpty() || onNewTab != null || onSharedView != null
    private fun hasAddTabButton(): Boolean = onNewTab != null
    private fun hasSharedViewButton(): Boolean = onSharedView != null
    ...
    private fun getTabsViewportWidth(totalWidth: Float = width.toFloat()): Float {
        ...
        val sharedButtonArea = if (hasSharedViewButton()) addButtonSizePx + tabSpacingPx else 0f
        return (
            totalWidth - tabHorizontalPaddingPx * 2f - addButtonArea - sharedButtonArea
        ).coerceAtLeast(0f)
    }
    ...
    fun release() {
        ...
        onNewTab = null
        onSharedView = null
        sharedViewActive = false
        sharedViewSelected = false
        sharedTerminalState = SharedHiddenTerminalState()
    ...
    private fun drawTabBar(canvas: Canvas) {
        ...
        val sharedButtonLeft: Float
        if (hasSharedViewButton()) {
            val sharedButtonRight =
                (addButtonLeft - if (hasAddTabButton()) tabSpacingPx else 0f).coerceAtLeast(0f)
            sharedButtonLeft = (sharedButtonRight - addButtonSizePx).coerceAtLeast(0f)
            val sharedButtonTop =
                rowTop + ((rowBottom - rowTop - addButtonSizePx) / 2f).coerceAtLeast(0f)
            val sharedButtonBottom = (sharedButtonTop + addButtonSizePx).coerceAtMost(rowBottom)
            sharedButtonRect.set(
                sharedButtonLeft,
                sharedButtonTop,
                sharedButtonRight,
                sharedButtonBottom
            ...
            val isSharedPressed =
                isTabTouchActive &&
                    !tabTouchMoved &&
                    activeTabTouchTarget is TabTouchTarget.SharedView &&
                    sharedButtonRect.contains(tabTouchX, tabTouchY)
            drawSharedViewButton(
                canvas,
                sharedButtonRect,
                isPressed = isSharedPressed,
                isActive = sharedViewActive
            )
        } else {
            sharedButtonLeft = addButtonLeft
        }

        val tabsLeft = tabHorizontalPaddingPx
        val tabsRight = if (hasSharedViewButton()) {
            (sharedButtonLeft - tabSpacingPx).coerceAtLeast(tabsLeft)
        } else if (hasAddTabButton()) {
        ...
        if (tabsRight <= tabsLeft) {
            tabHitSnapshot = TabHitSnapshot(
                ...
                sharedButtonRect = RectF(sharedButtonRect)
            )
            tabContentWidth = 0f
            tabScrollOffsetX = 0f
            if (!tabScroller.isFinished) {
    ...
    private fun drawSharedViewButton(
        canvas: Canvas,
        rect: RectF,
        isPressed: Boolean,
        isActive: Boolean
        ...
        val cx = rect.centerX()
        val cy = rect.centerY()
        val eyeHalfWidth = rect.width() * 0.27f
        val eyeHalfHeight = rect.height() * 0.16f
        tabIconPaint.color = if (isActive) 0xFF00E676.toInt() else Color.GRAY
        val eyeRect = RectF(
            cx - eyeHalfWidth,
            cy - eyeHalfHeight,
            cx + eyeHalfWidth,
        ...
        canvas.drawOval(eyeRect, tabIconPaint)
        tabPaint.color = tabIconPaint.color
        canvas.drawCircle(cx, cy, rect.width() * 0.075f, tabPaint)
    }
    ...
    private fun hitTestTabTarget(x: Float, y: Float): TabTouchTarget {
        ...
        if (hitSnapshot.sharedButtonRect.contains(x, y)) {
            return TabTouchTarget.SharedView
        }
    ...
    private fun isSameTabTarget(a: TabTouchTarget, b: TabTouchTarget): Boolean {
        return when {
            ...
            a is TabTouchTarget.SharedView && b is TabTouchTarget.SharedView -> true
    ...
    private fun executeTabTouchTarget(target: TabTouchTarget) {
        when (target) {
            ...
            is TabTouchTarget.SharedView -> onSharedView?.invoke()
    ...
    private data class SharedRenderLine(val text: String, val color: Int)

    private fun drawSharedTerminalContent(
        canvas: Canvas,
        contentTop: Float,
        contentBottom: Float,
        viewportWidth: Float
    ) {
        val state = sharedTerminalState
        val density = resources.displayMetrics.density
        val padding = 16f * density
        val lineHeight = (textPaint.textSize * 1.45f).coerceAtLeast(18f * density)
        val baselineOffset = -textPaint.fontMetrics.ascent
        val availableWidth = (viewportWidth - padding * 2f).coerceAtLeast(1f)

        val statusText =
            when {
                state.isActive -> context.getString(R.string.shared_terminal_active)
                state.operationId == null -> context.getString(R.string.shared_terminal_empty)
                else -> context.getString(R.string.shared_terminal_finished)
            }
        textPaint.color = if (state.isActive) 0xFF00E676.toInt() else Color.GRAY
        canvas.drawText(statusText, padding, contentTop + padding + baselineOffset, textPaint)

        val bodyLines = mutableListOf<SharedRenderLine>()
        if (state.command.isNotBlank()) {
            appendSharedWrappedLines(bodyLines, "\$ ${state.command}", 0xFF8BC34A.toInt(), availableWidth)
        }
        if (state.output.isNotBlank()) {
            appendSharedWrappedLines(bodyLines, state.output, Color.WHITE, availableWidth)
        }
        state.error?.takeIf { it.isNotBlank() }?.let { error ->
            appendSharedWrappedLines(bodyLines, error, 0xFFFF8A80.toInt(), availableWidth)
        }
        state.exitCode?.let { exitCode ->
            appendSharedWrappedLines(bodyLines, "[exit $exitCode]", Color.GRAY, availableWidth)
        }

        val bodyTop = contentTop + padding + lineHeight * 1.5f
        val visibleLineCount =
            ((contentBottom - bodyTop - padding) / lineHeight).toInt().coerceAtLeast(0)
        val visibleLines = bodyLines.takeLast(visibleLineCount)
        visibleLines.forEachIndexed { index, line ->
            textPaint.color = line.color
            canvas.drawText(
                line.text,
                padding,
                bodyTop + index * lineHeight + baselineOffset,
                textPaint
            )
        }
    }

    private fun appendSharedWrappedLines(
        destination: MutableList<SharedRenderLine>,
        text: String,
        color: Int,
        ...
        text.replace("\r\n", "\n").replace('\r', '\n').split('\n').forEach { logicalLine ->
            if (logicalLine.isEmpty()) {
                destination += SharedRenderLine("", color)
                return@forEach
            }
            var remaining = logicalLine
            while (remaining.isNotEmpty()) {
                val count = textPaint.breakText(remaining, true, availableWidth, null).coerceAtLeast(1)
                destination += SharedRenderLine(remaining.take(count), color)
    ...
    private fun drawTerminal(canvas: Canvas) {
        ...
        try {
            ...
            if (sharedViewSelected) {
                drawSharedTerminalContent(canvas, contentTop, contentBottom, viewportWidth)
                return
            }
    ...
    override fun onCreateInputConnection(outAttrs: EditorInfo?): InputConnection {
        ...
        return object : BaseInputConnection(this, true) {
            override fun commitText(text: CharSequence?, newCursorPosition: Int): Boolean {
                if (sharedViewSelected) return true
                text?.let {
    ...
    override fun onCheckIsTextEditor(): Boolean {
        return isFullscreenMode && !sharedViewSelected
    }
