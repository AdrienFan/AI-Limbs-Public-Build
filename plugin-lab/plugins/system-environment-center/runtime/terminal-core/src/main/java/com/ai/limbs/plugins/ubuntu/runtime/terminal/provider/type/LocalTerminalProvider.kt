package com.ai.limbs.plugins.ubuntu.runtime.terminal.provider.type
...
import com.ai.limbs.plugins.ubuntu.runtime.terminal.TerminalSession
...
class LocalTerminalProvider(
    ...
    private val shouldOfferDevelopmentEnvironmentPrompt: (String) -> Boolean = { false }
) : TerminalProvider {
        ...
        return withContext(Dispatchers.IO) {
            try {
                val command = buildVisibleSessionCommand(
                    shouldOfferDevelopmentEnvironmentPrompt(sessionId)
                )
    ...
    private suspend fun collectHiddenExecResult(
        ...
        while (System.currentTimeMillis() < deadline) {
            ...
            builder.append(chunk)
            val rawSnapshot = builder.toString()
            onOutputSnapshot?.let { observer ->
                runCatching { observer(extractHiddenExecOutput(rawSnapshot, token)) }
                    .onFailure { error ->
                        Log.w(TAG, "Hidden exec output observer failed", error)
                    }
            }
            extractHiddenExecPid(rawSnapshot, token)?.let { processGroupId ->
                shell.activeProcessGroupId.set(processGroupId)
            }
    ...
    private fun cancelHiddenExecCommand(rawOutput: String, token: String) {
        ...
        terminateHiddenExecProcessGroup(pid)
    }

    private fun terminateHiddenExecProcessGroup(pid: Long) {
        runCatching {
    ...
    private fun buildVisibleSessionCommand(showDevelopmentPrompt: Boolean): Array<String> {
        ...
        val mode = if (showDevelopmentPrompt) "prompt" else "skip"
        val startScript = "source \$HOME/common.sh && start_shell $mode"
