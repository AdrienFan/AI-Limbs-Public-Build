...
internal class LanerChatBridgeService(
    ...
    fun enqueue(
        ...
        attachments: List<LanerChatAttachmentInput> = emptyList(),
    ...
    private fun enqueueLocked(
        ...
        text: String,
        sender: String,
        attachments: List<LanerChatAttachmentInput>,
        ...
        val request =
            LanerChatRequest(
                ...
                chatMessageTimestamp = LanerChatTimestampAllocator.next(now)
            )
    ...
    suspend fun waitForNotification(
        ...
        val deadline = System.nanoTime() / 1_000_000L + boundedTimeout
        while (true) {
    ...
    fun completeTurn(turnId: String, replyId: String?, content: String): LanerChatTurnReplyResult {
        ...
        val now = System.currentTimeMillis()
        val chatTimestamp =
            existing.chatMessageTimestamp.takeIf { it > 0L } ?: LanerChatTimestampAllocator.next(now)
    ...
    private fun loadState(): LanerChatStoredState {
        if (!stateFile.isFile) return LanerChatStoredState()
    ...
    private fun commitState(updated: LanerChatStoredState) {
        ...
        check(parent.isDirectory || parent.mkdirs()) { "Unable to create Laner chat data directory" }
        val temp = File(parent, ".laner_chat_${UUID.randomUUID()}.tmp")
        try {
