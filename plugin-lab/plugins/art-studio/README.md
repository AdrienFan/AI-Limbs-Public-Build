...
新建支持 64–4096 像素边长、工程名称及透明背景；当前工程只有 RGB 8 位与一个初始绘画图层，不存储 ICC、DPI 或 Krita 原生色彩模型。画布底栏左侧的 ↩️/↪️ 调用与编辑菜单、兰儿能力相同的撤销/重做历史；没有可撤销或可重做操作时按钮置灰，忙碌时暂停操作。右侧“居中”只重置缩放、旋转、平移；旁边“全屏”通过 host.ui.presentation@1 控制页面；宿主未确认模式请求时，对话框保留并显示返回的错误。左右抽屉默认收起；抽屉打开后，点击画布区域或侧栏按钮均可收起，点击抽屉内的控件不会误触画布；右侧依次为多功能拾色器、真实图层管理、尚未实现的笔刷预设。图层与笔刷预设各有最大化按钮；最大化其中一块时，另外两块只留下固定高度的标题栏，当前块占据剩余高度。点击折叠的图层栏可切换到图层最大化，点击折叠的拾色器栏可恢复三段布局；最大化按钮再次点击也恢复三段布局。左抽屉暂不显示旧按钮。菜单栏文件和编辑以外的九项目前只有标题。
