...
新建工程先选像素宽高（64–4096）、工程名称和背景，再自动创建并选中第一个绘画图层；`document.create` 的可选 `name` 参数与页面共用同一创建逻辑。原右上角“⋮”菜单及其弹出项已移除；文件 → 新建现已接入，打开、保存及导出的底层能力保留，待逐项接入顶部菜单。画布左上角不叠放工程状态提示；默认收起图层、属性和历史面板，把高度留给画布。“画布适合窗口”只重置显示缩放，不改变图片像素。创建工程后，画布左右各显示 20dp 的抽屉把手；抽屉默认收起，展开一侧会收起另一侧。左抽屉暂不显示旧工具按钮，右抽屉当前只提供空面板；底部颜色、笔宽、不透明度和图层等原有操作仍可使用。页面的当前选中图层读取共享工程状态；AI 的 `layer.select` 能同步到页面。透明画布显示棋盘底，画笔颜色可直接输入 `#AARRGGBB`。PNG、JPEG 的页面导出处理流程已保留，入口待接入顶部菜单后会打开系统保存文件选择器。
