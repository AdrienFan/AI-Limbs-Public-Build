...
新建工程自动创建并选中绘画图层。固定工程栏、侧边工具区和可收起的图层、属性、历史面板让手机上保留画布空间。页面的当前选中图层读取共享工程状态；AI 的 `layer.select` 能同步到页面。透明画布显示棋盘底，画笔颜色可直接输入 `#AARRGGBB`。PNG、JPEG 页面导出会打开系统保存文件选择器。
