...
internal object DeveloperGuideContent {
    val sections = listOf(
        ...
        GuideSection(
            ...
            title = "4. 二级扩展 .ailx",
            description = "AIL_EXTENSION_V1：Hub 只负责新 .ailx 准入验证，Host Child Runtime 负责已安装子插件的运行与生命周期。",
            lines = listOf(
                ...
                "父插件通过 host.childExtensions.publishPoint() 声明 point、api、allowedHostCapabilities 与 binder；父插件停用或 Point 消失时 Host Child Runtime 必须将对应子插件 BLOCKED/停止。",
        ...
        GuideSection(
            ...
            lines = listOf(
                ...
                "父插件只有在 child_slots.<slot>.points 中显式列出 Extension Point，子插件才可向该 Slot 贡献 UI；Host Child Runtime 使用已准入 .ailx 的可信身份绑定 extensionId、parent、point，并在停用、卸载、失败或停止时自动撤销贡献。",
