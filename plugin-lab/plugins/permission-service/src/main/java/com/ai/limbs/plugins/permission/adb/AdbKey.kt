...
class AdbKey(private val adbKeyStore: AdbKeyStore, name: String) {
    ...
    companion object {
        ...
        private const val ENCRYPTION_KEY_ALIAS = "ai_limbs.permission_service.adb.encryption.v1"
    ...
    private fun getOrCreatePrivateKey(): RSAPrivateKey {
        ...
        val stored = adbKeyStore.get()
        if (stored != null) {
            val plaintext = requireNotNull(decrypt(stored, aad)) { "ADB key data is invalid" }
            return KeyFactory.getInstance("RSA").generatePrivate(PKCS8EncodedKeySpec(plaintext)) as RSAPrivateKey
        }
        ...
        adbKeyStore.put(requireNotNull(encrypt(created.encoded, aad)))
        return created
    }
