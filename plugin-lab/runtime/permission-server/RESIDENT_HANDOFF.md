...
Binder transaction 0x41494c uses descriptor ai_limbs.permission.resident.v1, protocol integer 1, then operation: 0 describe, 1 claim, 2 release, 3 prepare. Mutations include launch token, Core session and Core-owned lifetime Binder; release adds destination 0 (stop) or 1 (explicitly return to Host). Replies contain an exception header followed by JSON with server instance, server PID/UID and Core ownership identity.

Prepare links Core death and suppresses periodic Host Provider handoff. Claim requires the same prepared PID/session/Binder. Core death terminates the backend. Explicit release unlinks death and either terminates the backend or schedules Host handoff. Old in-flight Host results are applied only while no Core lease exists. Provider calls run on a single worker so they do not block the server main Looper.
