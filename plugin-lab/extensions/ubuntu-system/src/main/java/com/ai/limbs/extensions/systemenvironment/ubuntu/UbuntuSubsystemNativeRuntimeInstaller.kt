...
internal data class UbuntuSubsystemNativeRuntime(
    val abi: String,
    val directory: File,
    val ptyLibrary: File
)
 ...
 */
internal object UbuntuSubsystemNativeRuntimeInstaller {
    private const val REQUIRED_HOST_RUNTIME_API = 1
    private const val PTY_LIBRARY = "libsystem_environment_ubuntu_pty.so"

    fun prepare(host: InProcessPluginHost): UbuntuSubsystemNativeRuntime {
        val executableDirectory = resolveHostExecutableDirectory(host.nativeRuntime)
        val apk = host.runtimeEntryFile.canonicalFile
        require(apk.isFile) { "Ubuntu subsystem runtime APK is missing: ${apk.absolutePath}" }

        ZipFile(apk).use { zip ->
            val abi = selectAbi(zip)
            val root = File(
                host.applicationContext.codeCacheDir,
                "ailx-native/${sanitize(host.pluginId)}/${host.version}/$abi"
            ).apply { mkdirs() }
            val pty = extractPtyLibrary(zip, abi, root)
            return UbuntuSubsystemNativeRuntime(abi, executableDirectory, pty)
        }
    }
    ...
    private fun selectAbi(zip: ZipFile): String =
        Build.SUPPORTED_ABIS.firstOrNull { abi -> zip.getEntry("lib/$abi/$PTY_LIBRARY") != null }
            ?: error(
                "Ubuntu subsystem has no PTY library for supported ABIs ${Build.SUPPORTED_ABIS.joinToString()}"
            )
